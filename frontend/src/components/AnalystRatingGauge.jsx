...
import { ANALYST_TIERS, gaugeNeedleAngle, gaugeNeedleColor } from '../utils/analystGauge';

const formatPrice = (value) =>
  value != null ? `$${Number(value).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}` : null;

export const AnalystRatingGauge = ({ sentiment, loading, compact = false }) => {
  ...
  return (
    <div className={`analyst-gauge glass-panel panel-accent-cyan ${compact ? 'analyst-gauge--compact' : ''}`}>
      <div className="analyst-gauge-header">
        <Target size={18} aria-hidden="true" />
        <div>
          <h3 className="analyst-gauge-title">Analyst ratings</h3>
